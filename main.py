...
class Main:
    ...
    def __init__(self, debug, vidpath, output, stepSize, progressInterval):
        ...
        self.output = output
        self.detection = changedetection.ChangeDetection(stepSize, progressInterval, debug)
    ...
    def onProgress(self, percent, pos):
        ...
        print("progress: ~%d%% @ %d fps | about %s left" % (percent, fps, etaString))     
 
    def convertToPDF(self):
        self.imgs = []
        #self.dupeHandler.entries storing previous images yes
        for i in self.dupeHandler.entries:
            
            self.imgs.append(cv2.imencode('.jpg', i)[1].tostring())
        #print(len(self.imgs),len(self.dupeHandler.entries))
            
        with open(self.output, "wb") as f:
            f.write(img2pdf.convert(self.imgs))
        self.dupeHandler.entries=[]

    def start(self):
        self.dupeHandler.entries=[]
        #print(len(self.imgs),type(self.dupeHandler.entries))
...
main = Main(args['debug'], args['video'], args['output'],
            int(args['step-size']), args['progress-interval'])
main.start()
